...
def process_snapshot(args):
    ...
    if weighted and weights:
        s.plot_Aweightedslice(
            ...
            newfig=True, proj=proj, proj_fact=proj_fact, cblabel=r'$P\;[code\; units]$'
        )
    else:
        s.plot_Aslice(
...
if __name__ == '__main__':
    # Example usage - density-weighted temperature plot:
    make_movie(
        value='pres',
        ...
        proj_fact=0.05,
        res=1024,
        weighted=True,  # Enable density weighting
        weights='pres'   # Weight by temperature
    )