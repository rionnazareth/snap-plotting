...
def calc_T(data):
    temp = data['u'] * (GAMMA - 1) * data['mu']  * m_p  / k_B
    return temp
...
def load_snap_data(num,snappath=None,snapbase='snap_',advanced_xrays=False,verbose=False,default_ionisation='ionised'):
    ...
    if 'pressure' not in s.data:
        s.data['pressure'] = calc_P(s.data)
...
def plot_quad_axis(
    ...
    if weighted is None:
        s.axplot_Aslice(quad_ax,value=var,cmap=cmap,colorbar=False,divzero=divzero,divzero_centre=divzero_centre,vrange=ranges,axes=axes_sum,logplot=logplot,box=[plotsize,plotsize],center=plot_centre,proj=proj_on,proj_fact=proj_fact,res=res)
    else:
        s.axplot_Aweightedslice(quad_ax,value=var,weights=weighted,cmap=cmap,colorbar=False,divzero=divzero,divzero_centre=divzero_centre,vrange=ranges,axes=axes_sum,logplot=logplot,box=[plotsize,plotsize],center=plot_centre,proj=proj_on,proj_fact=proj_fact,res=res)
...
if __name__ == '__main__':
    ...
    BASE_PATH = '/mnt/ceph/users/sward/ACDC/runs/'
    SNAPBASE = 'snap_'
    SNAPFILETYPE = '.hdf5'
    ...
    snap_num = 10
    output_path = BASE_PATH + 'FR_allvars' + '/output/'
    ...
    plotsize = 2            # Size in kpc of one panel
    proj_on = True          # Whether to do a slice or a projection
    proj_fact = 0.1         # Fraction of plotsize to project through
    res = 256               # Pixels per panel
