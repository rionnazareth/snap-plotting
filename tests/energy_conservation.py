...
for label, cfg in RUNS.items():
    ...
    if cfg['has_cr']:
        ...
        if cre[label] is not None:
            ...
            # total energy including CRs
            # interpolate CR energy onto energy.txt time grid
            ecr_on_egrid = np.interp(
                energy[label]['time'],
                cre[label]['time'],
                cre[label]['ecr'],
            )
            energy[label]['ecr']    = ecr_on_egrid
            energy[label]['etotal'] = energy[label]['emech'] + ecr_on_egrid
        else:
