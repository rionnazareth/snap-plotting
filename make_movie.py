...
def process_snapshot(args):
    ...
    i, value, axes_plot, center, box, res, proj, proj_fact, vrange, cmap, logplot, snap_path = args
    ...
    # Create a temporary figure for this frame
    temp_fig = plt.figure(figsize=(8, 7), dpi=100)
    
    # Use newfig=True to let plot_Aslice create its own axes
    s.plot_Aslice(
    ...
    ax.set_title(f"{i*5} Myr", fontsize=12)
...
def make_movie(value, snap_path, snap_offset=0, num_snaps=10, 
    ...
    args_list = [
        (i, value, axes_plot, center, box, res, proj, proj_fact, vrange, cmap, logplot, snap_path)
        ...
        if save_path and movie_name:
            ...
            try:
                if ext == '.mp4':
                    ...
                    anim.save(output_file, fps=fps, writer='pillow')
                else:
                ...
                if ext == '.mp4':
                    ...
                    anim.save(gif_file, fps=fps, writer='pillow')
...
if __name__ == '__main__':
    ...
    make_movie(
        ...
        snap_offset=0,
        num_snaps=20,
        num_proc=128,  # Use 128 parallel processes
        save_path='/cosma8/data/dp317/dc-naza3/gasCloudNfw/plotting',
        movie_name='temperature_evolution.gif',
        fps=3,
        box=[100, 100],vrange=(1e6, 1e9),
        cmap='gnuplot',
        proj=True,
        proj_fact=0.1,
        res=1024
